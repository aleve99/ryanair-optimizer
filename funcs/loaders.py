...
from ryanair.types import Trip, OneWayFare, Stay

def save_airports(
        ryanair_graph: nx.MultiDiGraph,
        path: Path,
        filename: str = "airports.csv"
    ):

    codes = set()
    for node in ryanair_graph.nodes:
        codes.add(node)

    with open(path / filename, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['code', 'location', 'lng', 'lat'])
        writer.writeheader()
        for code, data in ryanair_graph.nodes(data=True):
            writer.writerow({
                'code': code,
...
def load_trips(
    ...
    df_stays = pd.read_csv(path / filename_stays, dtype={'trip_id': int, 'position': int, 'location': str, 'duration': int})
    ...
    # Pre-process fares DataFrame to convert timestamps to datetime
    df_fares['dep_time'] = pd.to_datetime(df_fares['dep_time'].apply(datetime.fromtimestamp))
    df_fares['arr_time'] = pd.to_datetime(df_fares['arr_time'].apply(datetime.fromtimestamp))
    ...
    for i, summary_row in df_summary.iterrows():
        ...
        stays = [
            Stay(location=row['location'], duration=timedelta(seconds=row['duration']))
        ...
        trips.append(Trip(
            ...
            total_duration=timedelta(seconds=summary_row['total_duration']),
            stays=tuple(stays)
        ))
    ...
    with open(path / filename_summary, mode='w', newline='') as file_summary:
        ...
        for i, trip in enumerate(trips):
            writer.writerow({
                ...
                "total_duration": int(trip.total_duration.total_seconds()),
                "num_flights": len(trip.flights),
                "departure_time": int(trip.flights[0].dep_time.timestamp()),
                "return_time": int(trip.flights[-1].arr_time.timestamp()),
...
def load_reachable_fares(
    ...
    for _, row in df.iterrows():
        element = OneWayFare(
            dep_time=datetime.fromtimestamp(row['dep_time']),
            arr_time=datetime.fromtimestamp(row['arr_time']),
            origin=row['origin'],
            destination=row['destination'],
            fare=float(row['fare']),
            left=int(row['left']),
    ...
    with open(path / filename, mode='w', newline='') as file:
        ...
        for fares_by_dest in fares_node_map.values():
            for fares in fares_by_dest.values():
                for fare in fares:
                    row = fare.to_dict()
                    row['dep_time'] = int(row['dep_time'].timestamp())
                    row['arr_time'] = int(row['arr_time'].timestamp())
